    ...
    <div className="container px-4 py-6 sm:py-8 md:py-10 mx-auto max-w-7xl">
      <div className="flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-10">
            ...
            {product.images.map((image, index) => (
              <button
                ...
                className={`w-16 h-16 sm:w-20 sm:h-20 overflow-hidden rounded-lg border-2 transition-colors ${
                  selectedImage === image ? "border-blue-500" : "border-gray-200 hover:border-gray-400"
                }`}
              >
        ...
        <div className="w-full lg:w-1/2 mt-6 lg:mt-0">
          ...
          {/* Product details */}
          <div className="mb-6">
            <div className="text-xl font-semibold mb-2">Price: ${product.price.toFixed(2)}</div>
            <div className="text-sm mb-2">Material: {product.material}</div>
            <div className="text-sm mb-2">Size: {product.size}</div>
            <div className="text-sm mb-2 flex items-center">
              Rating: <span className="ml-1 flex">{renderStars(product.rating)}</span>
            </div>
